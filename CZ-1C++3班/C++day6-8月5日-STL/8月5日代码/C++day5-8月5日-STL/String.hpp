...
namespace Cpang {
	...
	public:
		...
		void resize(int nsize, char val = '\0')
		{
			if (nsize <= m_size)
			{
				m_size = nsize;
		...
		iterator insert(iterator pos, iterator start, iterator end)
		{
			...
			int i;
			for (i = m_size - 1; i > ipos; i--)
			{
				m_val[i] = m_val[i - len];
			}
			iterator it;
			iterator tmp = pos;
			for (it = start; it != end; it++, tmp++)
			{
				*tmp = *it;
			}
			return pos;
		...
		iterator erase(iterator spos, iterator epos)
		{
			int len = epos - spos;
			int start = spos - m_val;
			m_size -= len;
			int i;
